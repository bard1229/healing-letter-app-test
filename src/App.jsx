...
const HealingLetterApp = () => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      ...
      if (currentUser) {
        ...
        loadLettersFromFirestore(currentUser.uid);
      } else {
        console.log('使用者未登入');
        setLetters([]);
      }
  ...
  // 從 Firestore 載入信件
  const loadLettersFromFirestore = async (userId) => {
    try {
      setLoading(true);
      ...
      setLetters(loadedLetters.reverse()); // 反轉順序,最舊的在前面
      console.log('載入了', loadedLetters.length, '封信件');
    } catch (error) {
      console.error('載入信件失敗:', error);
      alert('載入信件時發生錯誤,請重新整理頁面');
    } finally {
      setLoading(false);
    }
  };
...
一封給你的信 ✨`;
  ...
  const generateTrendAnalysis = (allLetters) => {
    ...
    let observation = '';
    if (stressedCount > 2) {
      observation = '你最近似乎承受著不少的壓力。記得要好好照顧自己,適時休息很重要。';
    } else if (sadCount > 2) {
      observation = '你的心情時常感到低落。我看見你每一次都願意面對,這份勇氣很珍貴。';
    } else {
      observation = '你的情緒有著不同的起伏,這是很自然的。每一種情緒都在告訴你一些訊息。';
    }
